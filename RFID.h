...
#define PICC_AUTHENT1A_B      0x62               // verificación Key A and B
...
#define MI_ERR                2
#define MI_AUTHA_ERR          3
#define MI_AUTHB_ERR          4
#define MI_SECTOR_RANGE_ERR   5
#define MI_BLOCK_RANGE_ERR    6
//------------------ MFRC522 registro---------------
...
{
  public:
    RFID(int chipSelectPin, int NRSTPD, unsigned char sectorKeyA[][6], unsigned char sectorKeyB[][6], unsigned char accessConditions[][3], unsigned char userData[][1]);
	...
	unsigned char read(unsigned char authent, unsigned char sector,unsigned char blockAddr, unsigned char *recvData);
	unsigned char write(unsigned char authent, unsigned char sector,unsigned char blockAddr, unsigned char *writeData);
	
	char writePasswordAccessConditionToCard(unsigned char authMod, unsigned sector);
	char writePasswordAccessConditionToCard(unsigned char authMod, unsigned char* oldSectorKey, unsigned sector);

	void setSectorKey(unsigned char sector, unsigned char sectorKeyA[6], unsigned char sectorKeyB[6], unsigned char accessConditions[3], unsigned char userData[1]);
	void setSectorKey(unsigned char sectorKeyA[][6], unsigned char sectorKeyB[][6], unsigned char accessConditions[][3], unsigned char userData[][1]);
	

	void reset();
	void halt();
	unsigned char* getSerNum() { return serNum;};
  ...
  private:
	...
	unsigned char _sectorKey[16][16];
	...
	unsigned char auth(unsigned char authMode, unsigned char BlockAddr, unsigned char* sectorKey);
};
