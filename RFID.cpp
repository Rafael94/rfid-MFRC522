...
///<summary>
///Initalisiert das RFID Objekt
///</summary>
RFID::RFID(int chipSelectPin, int NRSTPD, unsigned char sectorKeyA[][6], unsigned char sectorKeyB[][6], unsigned char accessConditions[][3], unsigned char userData[][1])
{
	...
	setSectorKey(sectorKeyA,sectorKeyB,accessConditions,userData);
 ...
 */
unsigned char RFID::auth(unsigned char authMode, unsigned char BlockAddr, unsigned char* sectorKey)
{
    unsigned char status;
 ...
 * MI_OK Valor de retorno: el retorno exitoso MI_OK
 */
unsigned char RFID::read(unsigned char authent, unsigned char sector, unsigned char blockAddr, unsigned char *recvData)
{
	if(sector<=0 || sector>16) return MI_SECTOR_RANGE_ERR;

	if(blockAddr<=0 || blockAddr>3) return MI_BLOCK_RANGE_ERR;
	
	unsigned char status;
    unsigned int unLen;

    recvData[0] = PICC_READ;
    recvData[1] = (blockAddr+((sector-1)*4))-1;
	;
	if(authent == PICC_AUTHENT1A_B ) {
		if (auth(PICC_AUTHENT1A, recvData[1], _sectorKey[sector-1]) != MI_OK) return MI_AUTHA_ERR;
		if (auth(PICC_AUTHENT1B, recvData[1], _sectorKey[sector-1]) != MI_OK) return MI_AUTHB_ERR;
	} else {
		if(authent==PICC_AUTHENT1A && auth(PICC_AUTHENT1A, recvData[1], _sectorKey[sector-1]) != MI_OK) {
				return MI_AUTHA_ERR;
		} else if(authent==PICC_AUTHENT1B && auth(PICC_AUTHENT1B,recvData[1], _sectorKey[sector-1]) != MI_OK) {
				return MI_AUTHB_ERR;
		}
	}
 ...
 * Valor de retorno: el retorno exitoso MI_OK
 */
unsigned char RFID::write(unsigned char authent, unsigned char sector, unsigned char blockAddr, unsigned char *writeData)
{
	if(sector<=0 || sector>16) return MI_SECTOR_RANGE_ERR;
    ...
    buff[0] = PICC_WRITE;
    buff[1] = (blockAddr+((sector-1)*4))-1;
	
	if(authent == PICC_AUTHENT1A_B ) {
		if (auth(PICC_AUTHENT1A, buff[1], _sectorKey[sector-1]) != MI_OK) return MI_AUTHA_ERR;
		if (auth(PICC_AUTHENT1B, buff[1], _sectorKey[sector-1]) != MI_OK) return MI_AUTHB_ERR;
	} else {
		if(authent==PICC_AUTHENT1A && auth(PICC_AUTHENT1A, buff[1], _sectorKey[sector-1]) != MI_OK) {
				return MI_AUTHA_ERR;
		} else if(authent==PICC_AUTHENT1B && auth(PICC_AUTHENT1B,buff[1], _sectorKey[sector-1]) != MI_OK) {
				return MI_AUTHB_ERR;
		}
	}
...
char RFID::writePasswordAccessConditionToCard(unsigned char authent, unsigned sector){
	unsigned char key[16] = {0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0x07,0x80,0x00,0x00,0x00,0x00,0x00,0x00,0x00};
	writePasswordAccessConditionToCard(authent, key, sector);
}


char RFID::writePasswordAccessConditionToCard(unsigned char authent, unsigned char* oldSectorKey, unsigned sector){
	...
	if(authent == PICC_AUTHENT1A_B ) {
		if (auth(PICC_AUTHENT1A, buff[1], oldSectorKey) != MI_OK) return MI_AUTHA_ERR;
		if (auth(PICC_AUTHENT1B, buff[1], oldSectorKey) != MI_OK) return MI_AUTHB_ERR;
	} else {
		if(authent==PICC_AUTHENT1A && auth(PICC_AUTHENT1A, buff[1], oldSectorKey) != MI_OK) {
				return MI_AUTHA_ERR;
		} else if(authent==PICC_AUTHENT1B && auth(PICC_AUTHENT1B,buff[1], oldSectorKey) != MI_OK) {
				return MI_AUTHB_ERR;
		}
	}
...
void RFID::setSectorKey(unsigned char sector, unsigned char sectorKeyA[6], unsigned char sectorKeyB[6], unsigned char accessConditions[3], unsigned char userData[1]) {
	for(short s=0;s<6;s++) {
		_sectorKey[sector][s]=sectorKeyA[s];
	}
	for(short s=6;s<9;s++) {
		_sectorKey[sector][s]=accessConditions[s];
	}
	for(short s=9;s<10;s++) {
		_sectorKey[sector][s]=userData[s];
	}
	for(short s=10;s<16;s++) {
		_sectorKey[sector][s]=sectorKeyB[s];
	}
}
void RFID::setSectorKey(unsigned char sectorKeyA[][6], unsigned char sectorKeyB[][6], unsigned char accessConditions[][3], unsigned char userData[][1]) {
	for(short i=0;i<16;i++) {
		for(short s=0;s<6;s++) {
			_sectorKey[i][s]=sectorKeyA[i][s];
		}
		for(short s=6;s<9;s++) {
			_sectorKey[i][s]=accessConditions[i][s];
		}
		for(short s=9;s<10;s++) {
			_sectorKey[i][s]=userData[i][s];
		}
		for(short s=10;s<16;s++) {
			_sectorKey[i][s]=sectorKeyB[i][s];
		}
	}
}